...
function Cart() {
  ...
  const onClearCart = () => {
    // window.confirt вызывает алерт с выбором (да===true/отмена===false)
    if (window.confirm('Вы действительно хотите очистить корзину?')) return dispatch(clearCart());
  }

  const onRemoveItem = (id) => {
    if (window.confirm('Вы действительно хотите очистить корзину?')) return dispatch(removeCartItem(id))
  }
      ...
      {totalCount !== 0
        ? <div className="cart">
          ...
          <div className="content__items">
            {addedPizzas.map((obj) => <CartItem {...obj} key={obj.id}
              totalPrice={items[obj.id].totalPrice}
              addEdCount={items[obj.id] && items[obj.id].items.length}
              onRemoveItem={onRemoveItem} plusCartItem={onPlusItem} minusCartItem={onMinusItem} />)}
          </div>
